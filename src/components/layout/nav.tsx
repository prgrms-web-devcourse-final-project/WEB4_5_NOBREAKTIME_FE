...
function Nav() {
    ...
    return (
        <nav className="group flex flex-col h-screen bg-[var(--color-sub-2)] text-[var(--color-main)] w-[83px] hover:w-[280px] transition-all duration-300 overflow-hidden p-4 flex flex-col gap-6">
            ...
            {/* 메뉴 아이템 */}
            <NavItem icon={<DashboardIcon />} label="Dashboard" href="/dashboard" active={pathname.startsWith("/dashboard")} />
            <NavItem icon={<BookmarkIcon />} label="Bookmark" href="/bookmark" active={pathname.startsWith("/bookmark")} />
            ...
            <NavItem icon={<QuestionIcon />} label="Grammar Learning" href="/grammar" active={pathname.startsWith("/grammar")} />
        </nav>
    );
}
...
function NavItem({
    ...
    const iconClass = `${active
        ...
        : "text-[var(--color-main)] opacity-60 group-hover:opacity-100"}`;
